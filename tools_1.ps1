...
function Disable-WindowsUpdate {
    Write-Host "`n[1] 正在下载禁用 Windows 更新工具..." -ForegroundColor Green
    try {
        irm https://cdn.jsdmirror.com/gh/huibsoft/static/wub.ps1 | iex
        Write-Host "? 已执行，请查看上方输出" -ForegroundColor Green
    } catch {
...
function Disable-SecurityCenter {
    Write-Host "`n[2] 正在下载禁用 Windows 安全中心工具..." -ForegroundColor Green
    try {
...
function Restore-ClassicMenu {
    Write-Host "`n[3] 正在下载恢复经典右键菜单工具..." -ForegroundColor Green
    try {
...
function Activate-Windows {
    Write-Host "`n[4] 正在下载 Windows 激活工具..." -ForegroundColor Green
    try {
...
function Clear-Memory {
    Write-Host "`n[5] 正在下载清理内存工具..." -ForegroundColor Green
    try {
...
do {
    ...
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "  [1] 禁用 Windows 更新" -ForegroundColor Green
    Write-Host "  [2] 禁用 Windows 安全中心" -ForegroundColor Green
    Write-Host "  [3] 恢复经典右键菜单" -ForegroundColor Green
    Write-Host "  [4] Windows 激活(KMS)" -ForegroundColor Green
    Write-Host "  [5] 内存清理工具" -ForegroundColor Green
